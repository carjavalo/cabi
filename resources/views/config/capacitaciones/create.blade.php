...
@section('header','Nueva Capacitación')
...
<div class="container-fluid px-4">
    <div class="d-flex justify-content-between align-items-end mb-4">
        <div>
            <h1 class="cap-header fw-bold" style="font-size:2rem;">Configurar Capacitación</h1>
            <p class="text-muted">Define los datos de la sesión y selecciona los usuarios citados.</p>
        </div>
    ...
    <form method="POST" action="{{ route('config.capacitaciones.store') }}" id="formCapacitacion">
        @csrf
            ...
            <div class="col-lg-5">
                <div class="cap-card mb-4">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="titulo" class="cap-input" placeholder="Ej: Seguridad y Salud en el Trabajo" value="{{ old('titulo') }}" required>
                        @error('titulo') <small class="text-danger">{{ $message }}</small> @enderror
                    </div>
                    <div class="mb-3">
                        <label class="cap-label">Descripción</label>
                        <textarea name="descripcion" class="cap-input" rows="3" placeholder="Descripción breve de la capacitación">{{ old('descripcion') }}</textarea>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <label class="cap-label">Fecha <span class="text-danger">*</span></label>
                            <input type="date" name="fecha" class="cap-input" value="{{ old('fecha') }}" required>
                            @error('fecha') <small class="text-danger">{{ $message }}</small> @enderror
                        </div>
                        <div class="col-6">
                            <label class="cap-label">Ubicación</label>
                            <input type="text" name="ubicacion" class="cap-input" placeholder="Salón de conferencias" value="{{ old('ubicacion') }}">
                        </div>
                    </div>
                    <div class="row g-3 mb-3">
                        <div class="col-6">
                            <label class="cap-label">Hora Inicio</label>
                            <input type="time" name="hora_inicio" class="cap-input" value="{{ old('hora_inicio') }}">
                        </div>
                        <div class="col-6">
                            <label class="cap-label">Hora Fin</label>
                            <input type="time" name="hora_fin" class="cap-input" value="{{ old('hora_fin') }}">
                        </div>
                    </div>
                    <div class="row g-3">
                        <div class="col-6">
                            <label class="cap-label">Instructor</label>
                            <input type="text" name="instructor" class="cap-input" placeholder="Nombre del instructor" value="{{ old('instructor') }}">
                        </div>
                        <div class="col-6">
                            <label class="cap-label">Capacidad Máxima</label>
                            <input type="number" name="capacidad_maxima" class="cap-input" placeholder="0 = sin límite" min="0" value="{{ old('capacidad_maxima', 0) }}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="col-lg-7">
                <div class="cap-card" style="min-height:100%;">
                    ...
                    <div class="position-relative mb-3">
                        ...
                        <input type="text" id="searchUsuarios" class="cap-input" style="padding-left: 2.5rem;" placeholder="Buscar participante por nombre, identificación o servicio...">
                    </div>
                    ...
                    <div id="usersList" style="max-height: 420px; overflow-y: auto; border: 1px solid #e3e1e8; border-radius: 1rem; padding: 0.5rem;">
                        ...
                        <label class="user-item w-100 mb-0" data-search="{{ mb_strtolower($u->name.' '.$u->apellido1.' '.$u->apellido2.' '.$u->identificacion.' '.$u->servicio) }}">
                            <input type="checkbox" name="usuarios[]" value="{{ $u->id }}" {{ in_array($u->id, old('usuarios', [])) ? 'checked' : '' }}>
                            @php
        ...
        <div class="d-flex justify-content-end mt-4 mb-4">
            ...
            <button type="submit" class="cap-gradient-btn">
                <i class="fas fa-save me-2"></i>Guardar Capacitación
            </button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    selectAll.addEventListener('change', function() {
        const visible = document.querySelectorAll('#usersList .user-item:not([style*="display: none"]) input[name="usuarios[]"]');
